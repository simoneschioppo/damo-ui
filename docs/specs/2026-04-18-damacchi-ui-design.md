...
Il repo è un **pnpm workspace monorepo leggero**: la lib `@damo/ui` è l'unico pacchetto pubblicato. Playground Next.js e test e2e Playwright vivono nel repo ma non vengono rilasciati.
...
│     ├─ package.json                 ← depends on: "@damo/ui": "workspace:*"
│     ├─ next.config.ts
│     ├─ tailwind.config.ts           ← estende tailwind.preset di @damo/ui
│     ├─ app/
...
import { Button } from '@damo/ui/button'

// Barrel (comodità)
import { Button, Dialog } from '@damo/ui'
