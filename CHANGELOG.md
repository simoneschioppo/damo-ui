...
- README polish for publication (audit PR-R): corrected Card variant count (3 → 5), corrected atomic icon count (30 → 31), moved `MemphisShape` from "Cards" to "Foundations" (it's a decoration primitive, not a card wrapper), renamed "Tier 1 signature" → "Core actions & surfaces" (user-facing label), added a Layout-composition snippet (`AppShell` + `AppTopBar` + `Sidebar` + `PageHeader`), aligned tagline between root and npm READMEs, added a CHANGELOG link in the npm README's 0.x preview disclaimer, and linked the npm README's "Tailwind v3 preset" line to the matching repo-README section.
