...
const SHARED_PROPS: ReadonlyArray<PropDef> = [
  { name: 'storageKey', type: 'string', description: 'localStorage key used to persist the choice.' },
  { name: 'attribute', type: 'string', description: 'data-attribute mirrored on <html>.' },
  { name: 'defaultValue', type: 'string', description: 'Initial value when nothing is persisted yet.' },
]

const THEME_PROPS: ReadonlyArray<PropDef> = [
  { name: 'options', type: 'ThemeOption[]', defaultValue: '[{ light }, { dark }]', description: 'Buttons rendered in the toggle group.' },
  ...SHARED_PROPS,
]

const PALETTE_PROPS: ReadonlyArray<PropDef> = [
  { name: 'options', type: 'PaletteOption[]', required: true, description: 'List of palettes the consumer wants to expose. No implicit default — caller decides.' },
  ...SHARED_PROPS,
]

export const metadata = { title: 'Theme Switchers — Axolab' }
  ...
  return (
    <article>
      ...
      <p className="text-lg text-muted-foreground max-w-[60ch] mb-10">
        Three sibling switchers that flip a <code className="font-mono">data-*</code> attribute
        on <code className="font-mono">&lt;html&gt;</code> and persist the choice in
        localStorage. They are independent and orthogonal — switching one never resets the others.
      </p>
      ...
      <p className="text-foreground/80 mb-3">
        Toggles <code className="font-mono">data-density=&quot;compact|normal|comfortable&quot;</code>.
        Drives <code className="font-mono">--density-scale-y</code>, which is multiplied into
        every component's vertical spacing.
      </p>
      <div className="my-6 border-2 border-memphis bg-background shadow-memphis px-6 py-8 flex justify-center">
        <DensitySwitcher />
      </div>
